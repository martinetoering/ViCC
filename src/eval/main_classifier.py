...
write_log, calc_topk_accuracy, denorm, batch_denorm, Logger, FastDataLoader, \
ProgressMeter, neq_load_customized, bool_flag
...
def parse_args():
    ...
    parser.add_argument('--backbone_ratio', default=10, type=int, help='ratio with which the backbone lr should be smaller than lr')
...
def main(args):
    ...
    if args.train_what == 'last':
        ...
        for name, param in model.named_parameters():
            if 'backbone' in name:
                params.append({'params': param, 'lr': args.lr/args.backbone_ratio})
            else:
                params.append({'params': param})
    ...
    # save args
    if not args.test:
        with open(args.img_path + "/../args-{}.txt".format(args.name_prefix.replace("/", "-")), 'w') as f:
    ...
    wandb.init(project=args.model+"-"+args.train_what, config=vars(args), name=args.name_prefix)
...
def train_one_epoch(data_loader, model, criterion, optimizer, transforms_cuda, device, epoch, args):
    ...
    for idx, (input_seq, target) in enumerate(data_loader):
        ...
        if idx % args.print_freq == 0:
            ...
            wandb.log({"train/local/loss": losses.local_avg, "train/local/top1": top1_meter.local_avg, 
                        "train/local/top5": top5_meter.local_avg, "train/step": args.iteration})
    ...
    wandb.log({"train/global/loss": losses.avg, "train/global/top1": top1_meter.avg, 
                "train/global/top5": top5_meter.avg, "train/global_step": epoch})
...
def validate(data_loader, model, criterion, transforms_cuda, device, epoch, args):
    ...
    if args.tb_background:
        ...
        args.writer_val.add_scalar('global/top1', top1_meter.avg, epoch)
        args.writer_val.add_scalar('global/top5', top5_meter.avg, epoch)

    wandb.log({"val/global/loss": losses.avg, "val/global/top1": top1_meter.avg, 
                "val/global/top5": top5_meter.avg, "val/global_step": epoch})
...
def test_retrieval(model, criterion, transforms_cuda, device, epoch, args):
    ...
    with torch.no_grad():
        ...
        train_dataset = d_class(lmdb_root=args.dataset_root,
                            mode='train', 
                            transform=transform, 
        ...
        test_dataset = d_class(lmdb_root=args.dataset_root,
                            mode='test', 
                            transform=transform, 
                            num_frames=args.num_seq*args.seq_len,
                            ds=args.ds,
        ...
        train_loader = FastDataLoader(train_dataset,
                                      batch_size=1,
                                      sampler=train_sampler,
                                      shuffle=False,
                                      num_workers=args.workers,
                                      pin_memory=True)
        test_loader = FastDataLoader(test_dataset,
                                      batch_size=1,
                                      sampler=test_sampler,
                                      shuffle=False,
                                      num_workers=args.workers,
        ...
        if os.path.exists(os.path.join(os.path.dirname(args.test), dirname, '%s_test_feature.pth.tar' % args.dataset)): 
            ...
            test_vname = []
            sample_id = 0 
            for idx, (input_seq, target) in tqdm(enumerate(test_loader), total=len(test_loader), position=0, leave=None):
                B = 1
...
def get_data(transform, mode, args):
    ...
    print("args.dataset root:", args.dataset_root)
    if args.dataset == 'ucf101':
        dataset = UCF101LMDB(lmdb_root=args.dataset_root,
            mode=mode, transform=transform, 
            num_frames=args.seq_len*args.num_seq, ds=args.ds, which_split=args.which_split,
            return_label=True)

    elif args.dataset == 'ucf101-f':
        dataset = UCF101Flow_LMDB(lmdb_root=args.dataset_root,
            mode=mode, transform=transform, 
            num_frames=args.seq_len*args.num_seq, ds=args.ds, which_split=args.which_split,
            return_label=True)

    elif args.dataset == 'hmdb51':
        dataset = HMDB51LMDB(lmdb_root=args.dataset_root,
            mode=mode, transform=transform, 
            num_frames=args.seq_len*args.num_seq, ds=args.ds, which_split=args.which_split,
            return_label=True)
    elif args.dataset == 'hmdb51-f':
        dataset = HMDB51Flow_LMDB(lmdb_root=args.dataset_root,
            mode=mode, transform=transform, 
            num_frames=args.seq_len*args.num_seq, ds=args.ds, which_split=args.which_split,
            return_label=True)
...
def set_path(args):
    if args.resume: exp_path = os.path.dirname(os.path.dirname(args.resume))
        ...
        exp_path = '{args.prefix}/eval/{args.name_prefix}/{args.dataset}-{args.img_dim}_sp{args.which_split}_{args.model}_{args.net}\
_bs{args.batch_size}_lr{args.lr}_dp{args.dropout}_wd{args.wd}_seq{args.num_seq}_len{args.seq_len}_ds{args.ds}_\
