...
# From SWAV https://github.com/facebookresearch/swav
...
def save_checkpoint(state, is_best=0, gap=1, prefix='', filename='models/checkpoint.pth.tar', save_epoch=False, keep_all=False):
    torch.save(state, filename)
    last_epoch_path = os.path.join(os.path.dirname(filename),
                                   '{}epoch{}.pth.tar'.format(prefix, str(state['epoch']-gap)))
    if not keep_all:
    ...
    if is_best:
        ...
        for f in os.listdir(os.path.dirname(filename)):
            if f.startswith("{}model_best_".format(prefix)):
                past_best.append(os.path.join(os.path.dirname(filename), f))
        past_best = sorted(past_best, key=lambda x: int(''.join(filter(str.isdigit, x))))
        if len(past_best) >= 5:
            try: os.remove(past_best[0])
            except: pass
        torch.save(state, os.path.join(os.path.dirname(filename), '{}model_best_epoch{}.pth.tar'.format(prefix, str(state['epoch']))))

    ########################
    # save extra epochs
    if save_epoch:
        torch.save(state, os.path.join(os.path.dirname(filename), '{}save_epoch{}.pth.tar'.format(prefix, str(state['epoch']))))
    ########################
...
def denorm(device, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
    assert len(mean)==len(std)==3
    inv_mean = torch.as_tensor([-mean[i]/std[i] for i in range(3)]).to(device)
    inv_std = torch.as_tensor([1/i for i in std]).to(device)
