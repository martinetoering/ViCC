...
class UCF101LMDB_2CLIP(object):
    def __init__(self, lmdb_root, 
                 ...
                 return_source=False):
        self.root = root
        self.db_path = os.path.join(lmdb_root, db_path)
...
class HMDB51LMDB(UCF101LMDB):
    def __init__(self, lmdb_root, 
                 ...
                 db_path='HMDB51/hmdb51_frame.lmdb',
                 **kwargs):
        super(HMDB51LMDB, self).__init__(lmdb_root=lmdb_root, root=root, db_path=db_path, **kwargs)


class UCF101Flow_LMDB_2CLIP(object):
    def __init__(self, lmdb_root, 
                 root='%s/../process_data/data/ucf101' % os.path.dirname(os.path.abspath(__file__)), 
                 db_path='UCF101/ucf101_tvl1_frame.lmdb',
                 transform=None, mode='val',
                 num_frames=32, ds=1, which_split=1,
                 return_label=False,
                 return_path=False,
...
class HMDB51Flow_LMDB(UCF101Flow_LMDB):
    def __init__(self, lmdb_root, 
                 root='%s/../process_data/data/hmdb51' % os.path.dirname(os.path.abspath(__file__)), 
                 db_path='HMDB51/hmdb51_tvl1_frame.lmdb',
                 **kwargs):
        super(HMDB51Flow_LMDB, self).__init__(lmdb_root=lmdb_root, root=root, db_path=db_path, **kwargs)


class UCF101_2STREAM_LMDB_2CLIP(object):
    def __init__(self, lmdb_root, 
                 root='%s/../process_data/data/ucf101' % os.path.dirname(os.path.abspath(__file__)), 
                 db_path_flow='UCF101/ucf101_tvl1_frame.lmdb',
                 db_path_rgb='UCF101/ucf101_frame.lmdb',
                 transform=None, mode='val',
                 num_frames=32, ds=1, which_split=1,
                 return_label=False, 
                 return_path=False,
                 return_source=False):
        self.root = root
        self.db_path_flow = os.path.join(lmdb_root, db_path_flow)
        self.db_path_rgb = os.path.join(lmdb_root, db_path_rgb)
        self.transform = transform
        self.mode = mode 
        self.num_frames = num_frames
        self.ds = ds
    ...
    def __getitem__(self, index):
        ...
        if self.return_source:
            source_id = self.video_source[vname.split('/')[-1][0:-4]]
            if self.return_path and self.return_label:
                vid = self.encode_action(vlabel)
                return (seq1, seq2), source_id, vid, vpath
            if self.return_label:
                vid = self.encode_action(vlabel)
...
class Kinetics_2STREAM_LMDB_2CLIP(object):
    def __init__(self, 
                 root, db_path_flow, db_path_rgb, filename_flow, filename_rgb,
                 transform=None, mode='val',
                 num_frames=32, ds=1,
                 return_label=False, 
                 return_path=False,
